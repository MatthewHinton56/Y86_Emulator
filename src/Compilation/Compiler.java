...
public class Compiler {
	...
	/**
	 * Complies the .ys file and produces a sequence of maps for initializing
	 * 
	 * @param input          the input file to compile
	 * @param compilerOutput A string pointer to store the compiler output
	 * @return the compiled output
	 */
	public static String compile(String input, String[] compilerOutput) {
		preprocessor(input);
		compiled = true;
		...
		for (Line l : inputLines) {
			...
			case "nop":
				if (l.splitLine.length > 1)
					throw new IllegalArgumentException("Invalid instruction argument - " + l.splitLine[0]
			...
			case "popq":
				if (l.splitLine.length != 2)
					throw new IllegalArgumentException("Invalid instruction argument - " + l.splitLine[0]
							+ " requires only 1 register argument\n" + "Error occured on the line: " + l.line);
			...
			case "jl":
				...
				if (!TAG_TO_ADDRESS.containsKey(l.splitLine[1]))
					throw new IllegalArgumentException("Invalid instruction argument - Invalid Tag: " + l.splitLine[1]
							+ "\n" + "Error occured on the line: " + l.line);
				instruction = InstructionBuilder.getInstruction(l.splitLine[0], null, null,
						new DoubleWord(TAG_TO_ADDRESS.get(l.splitLine[1]), false), l.line);
				outputLine += convertArrayToString(instruction);
				COMPILED_INSTRUCTIONS.put(Long.parseLong(l.address, 16), instruction);
				break;
			case "irmovq":
			...
			default:
				if (l.splitLine[0].contains(":")) {
					...
					throw new IllegalArgumentException(
							"Invalid instruction: " + l.splitLine[0] + "\n" + "Error occured on the line: " + l.line);
			}
			outputLine += " " + l.line + "\n";
			compilerOutput[0] += (l.line + " ==> " + outputLine + "\n");
			output += outputLine;
		}
		return output;
	}
	...
	/**
	 * Parses an irmovq line
	 * 
	 * @param output the output pre irmovq
	 * @param l      the line to process
	 * @return the output post irmovq
	 */
	private static String irmovq(String output, Line l) {
		DoubleWord dw;
		if (l.splitLine.length != 3)
			throw new IllegalArgumentException(
					"Invalid instruction argument - irmovq requires 1 register and 1 immediate\n"
							+ "Error occured on the line: " + l.line);
		String[] instruction;
		if (TAG_TO_ADDRESS.containsKey(l.splitLine[1])) {
			...
			try {
				...
				throw new IllegalArgumentException("Invalid instruction argument - Invalid Hex argument: "
						+ l.splitLine[1] + "\n" + "Error occured on the line: " + l.line);
			}
			instruction = InstructionBuilder.getInstruction(l.splitLine[0], "No register", l.splitLine[2], dw, l.line);
			output += convertArrayToString(instruction);
			...
			try {
				...
				throw new IllegalArgumentException("Invalid instruction argument - Invalid Integer argument: "
						+ l.splitLine[1] + "\n" + "Error occured on the line: " + l.line);
			}
			instruction = InstructionBuilder.getInstruction(l.splitLine[0], "No register", l.splitLine[2], dw, l.line);
			output += convertArrayToString(instruction);
	...
	/**
	 * Parses an rmmovq line
	 * 
	 * @param output the output pre rmmovq
	 * @param l      the line to process
	 * @return the output post rmmovq
	 */
	private static String rmmovq(String output, Line l) {
		if (l.splitLine.length != 3)
			throw new IllegalArgumentException(
					"Invalid instruction argument - rmmovq requires 1 register and 1 memory function\n"
							+ "Error occured on the line: " + l.line);
		if (!l.splitLine[2].contains("(") || !l.splitLine[2].contains(")"))
			throw new IllegalArgumentException(
					"Invalid instruction argument - The second argument of mrmovq must be the memory function\n"
							+ "Error occured on the line: " + l.line);
		String offset;
		String rB;
		String[] instruction;
	...
	/**
	 * Parses an mrmovq line
	 * 
	 * @param output the output pre mrmovq
	 * @param l      the line to process
	 * @return the output post mrmovq
	 */
	private static String mrmovq(String output, Line l) {
		...
		if (!l.splitLine[1].contains("(") || !l.splitLine[1].contains(")"))
			throw new IllegalArgumentException(
					"Invalid instruction argument - The first argument of mrmovq must be the memory function\n"
							+ "Error occured on the line: " + l.line);
		String offset;
		String rB;
		String[] instruction;
	...
	/**
	 * Performs the quad directive in the pre compile process
	 * 
	 * @param output the output pre quad directive
	 * @param l      the line to process
	 * @return the output post the quad directive
	 */
	...
	/**
	 * creates a map of tags assigned with an address Each line is associated with
	 * an address and directive are processed
	 * 
	 * @param input the file to be compiled
	 */
	...
	/**
	 * Calculates the next address for the pre compilation process
	 * 
	 * @param address     the address pre instruction
	 * @param instruction the instruction to process
	 * @return the address post instruction
	 */
	private static long calculateAddress(long address, String instruction) {
	...
	/**
	 * performs the pos directive during processing
	 * 
	 * @param address   the address pre position direction
	 * @param sLine     the line to be displayed
	 * @param splitLine the line split into an array
	 * @return the addres post position directive
	 */
	private static long posPreProcess(long address, String sLine, String[] splitLine) {
		if (splitLine.length < 2)
			throw new IllegalArgumentException(
					"Invalid assembler directive argument - .pos requires an address argument.\n"
							+ "Error occured on the line: " + sLine);
		if (splitLine[1].startsWith("0x")) {
			try {
				address = Integer.parseInt(splitLine[1].substring(2), 16);
			} catch (NumberFormatException e) {
				throw new IllegalArgumentException("Invalid assembler directive argument - Invalid Hex argument: "
						+ splitLine[1] + " for .pos.\n" + "Error occured on the line: " + sLine);
			}
		} else {
			try {
				...
				throw new IllegalArgumentException("Invalid assembler directive argument - Invalid Integer argument: "
						+ splitLine[1] + " for .pos.\n" + "Error occured on the line: " + sLine);
			}
		}
		inputLines.add(new Line(Long.toHexString(address), splitLine, sLine));
	...
	/**
	 * performs the align directive during processing
	 * 
	 * @param address   the address pre align
	 * @param sLine     the line to display
	 * @param splitLine the line split into an array
	 * @return the address post align
	 */
	private static long alignPreProcess(long address, String sLine, String[] splitLine) {
		if (splitLine.length < 2)
			throw new IllegalArgumentException(
					"Invalid assembler directive argument - .align requires an address argument.\n"
							+ "Error occured on the line: " + sLine);
		try {
	...
	/**
	 * The line class, used to represent a line during compilation. Stores an
	 * address, the line as an array, and the original line post comment removal
	 * 
	 * @author Matth
	 *
	 */
	private static class Line {
		...
		/**
		 * The line constructor to create line representation
		 * 
		 * @param address   the address of the line
		 * @param splitLine the line split in an array
